...
class CartService
{
    ...
    public function addItemToCart(Product $product, int $quantity = 1, $optionIds = null)
    {
        ...
        {
            $optionIds = $product->variations
                        ->mapWithKeys(fn(Variation $type) => [$type->id => $type->options[0]?->id])
                        ->toArray();
        }
        ...
        try{
            if($this->cachedCartItems === null)
            {
                ...
                foreach($cartItems as $cartItem){
                    ...
                    $optionInfo = [];
                    $options = VariationOption::with('variation')
        ...
        $cartItems = CartItem::where('user_id', $userId)
                    ...
                    ->map(function($cartItem){
                        return[
                            ...
                            'quantity'=>$cartItem->quantity,
                            'price'=>$cartItem->price,
                            'option_ids' => $cartItem->variation_option_ids,
                        ];
                    })
                    ->toArray();
    ...
    public function moveCartItemsToDatabase($userId):void
    {
        ...
        foreach($cartItems as $itemKey => $cartItem)
        {
            $existingItem = CartItem::where('user_id', $userId)
                        ...
                        ->where('variation_option_ids', json_encode($cartItem['option_ids']))
                        ->first();
            ...
            if($existingItem)
            {
                ...
                CartItem::create([
                    ...
                    'quantity'=>$cartItem['quantity'],
                    'price'=>$cartItem['price'],
                    'variation_option_ids' => $cartItem['option_ids']
                ]);
            }
        }
